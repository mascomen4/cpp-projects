...
{
    /*
     * Initializes variables, for each mesh (triangle) finds the image that maps the best on the mesh
     * Then stores it's data to call function saveOBJwithMTL
     * Note: in my task, I'm already given the uv coords of the vertexes.
    */
    ...
    for( size_t mesh_i = 0; mesh_i < mesh_num; mesh_i++ ) {
        ...
        // Fills the data for the mesh such as uv coord, points index,
...
*/
{
    ...
    for(size_t p_i = 0; p_i < 3; p_i++) {
        ...
        cv::Mat X_img = worldToImg(X_w, img_i); // results in (x,y,z) point
